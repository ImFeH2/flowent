...
const ELK_DESIRED_EDGE_LENGTH = 220;
const ELK_ITERATION_LIMIT = 300;

const ELK_LAYOUT_OPTIONS = {
  "elk.algorithm": "stress",
  "elk.padding": "[left=0, top=0, right=0, bottom=0]",
  "org.eclipse.elk.stress.desiredEdgeLength": `${ELK_DESIRED_EDGE_LENGTH}`,
  "org.eclipse.elk.stress.iterationLimit": `${ELK_ITERATION_LIMIT}`,
} as const;
...
async function layoutComponent(nodes: Node[], edges: Edge[]): Promise<Node[]> {
...
export async function getAgentGraphLayoutedElements(
  nodes: Node[],
  edges: Edge[],
  ...
  for (const startNode of orderedNodes) {
    ...
    const layoutedNodes = await layoutComponent(componentNodes, componentEdges);
