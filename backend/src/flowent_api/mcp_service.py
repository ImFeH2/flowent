...
def _build_root_uri(path: str) -> str:
    from flowent_api.settings import resolve_path

    return resolve_path(path).as_uri()


def _build_roots_for_agent(agent: Agent) -> list[dict[str, str]]:
    from flowent_api.settings import get_runtime_working_dir_path, resolve_path
...
class MCPService:
    ...
    def create_or_update_server(
        ...
        *,
        ...
        from flowent_api.settings import _build_mcp_server_config
