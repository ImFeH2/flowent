...
- The AI is responsible for the full execution path: reading the spec commit, understanding the affected modules, planning, implementing, reviewing, committing, and advancing the aligned ref.
...
8. Choose `1-10` subagents based on the actual impact surface and have them check concrete alignment risks instead of doing generic review.
9. Fix any misalignment they find and re-check as needed.
...
14. In that summary, explicitly prefer:
    - what the user will now see behave differently
    - what the user can now do that was missing or broken before
    - what result is now correct, complete, or more predictable

## Subagent policy

- Choose subagents dynamically from the actual change surface.
- Prefer disjoint review angles such as behavior semantics, adjacent workflow impact, runtime/API constraints, or UI consistency.
- Do not launch fixed review bundles by habit.
- Do not advance the alignment pointer while any known spec misalignment remains.
- The final summary should answer the practical question “what did this code change actually make happen” rather than narrating files or internals.
