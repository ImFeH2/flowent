...
from app.access import ensure_access_bootstrap, is_access_configured, set_access_code
...
def test_ensure_access_bootstrap_generates_and_logs_access_code() -> None:
    ...
    assert any(
        f"Autopoe admin access code: {generated_code}" in message
...
def test_ensure_access_bootstrap_logs_existing_persisted_access_code() -> None:
    ...
    assert any(
        "Autopoe admin access code: TEST-ACCESS-CODE" in message for message in messages
    )
