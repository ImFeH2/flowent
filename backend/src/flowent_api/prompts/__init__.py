...
from flowent_api.prompts.common import DEFAULT_AGENT_ROLE_PROMPT, compose_system_prompt
...
def _get_assistant_role_prompt(*, settings, role_name: str) -> str:
    from flowent_api.settings import STEWARD_ROLE_NAME, find_role
...
def get_system_prompt(config: NodeConfig) -> str:
    ...
    if config.node_type == NodeType.ASSISTANT:
        ...
        if config.role_name:
            from flowent_api.settings import find_role
