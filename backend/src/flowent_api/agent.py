...
from flowent_api.assistant_commands import build_assistant_help_text
from flowent_api.events import event_bus
from flowent_api.image_assets import create_image_asset, require_image_asset
...
from flowent_api.settings import get_settings
...
class Agent:
    ...
    def _persist_workspace_node(self) -> None:
        if self.node_type != NodeType.ASSISTANT and not self.config.tab_id:
            return
        from flowent_api.models import GraphNodeRecord
    ...
    def get_contact_ids_snapshot(self) -> list[str]:
        from flowent_api.graph_service import get_tab_leader_id
    ...
    def get_contacts_info(self) -> list[dict[str, Any]]:
        from flowent_api.graph_service import is_tab_leader
        from flowent_api.registry import registry
    ...
    def _get_llm_retry_429_delay(self, status_code: int | None) -> float:
        if status_code != 429:
            return 0.0
        from flowent_api.settings import find_provider, find_role
    ...
    def _get_effective_model_source(self) -> ResolvedModelSource:
        from flowent_api.settings import find_provider, find_role, resolve_model_info
    ...
    def _record_request_stats(
        ...
        *,
        ...
        from flowent_api.stats_service import RequestRecordInput, stats_store
    ...
    def _run_compact_with_stats(
        ...
        *,
        ...
        from flowent_api.stats_service import CompactRecordInput, stats_store
    ...
    def _get_effective_output_budget_tokens(self) -> int:
        from flowent_api.settings import find_role, merge_model_params
